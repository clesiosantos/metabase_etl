...
final class ChangesJob {
  ...
  public static function run(PDO $src, PDO $dst, string $mode, int $windowFullDays, int $batchSize): void {
    ...
    try {
      ...
      if ($idsSelected > 0) {
        TagsJobs::refreshChangeLinks($src, $dst, $ids, $loadTimestamp);
      }
      ...
      if ($mode === 'full') {
        ChangesLoader::pruneInactive($dst, $loadTimestamp);
        TagsLoader::pruneBridgeLinks($dst, 'Change', $loadTimestamp);
      }
